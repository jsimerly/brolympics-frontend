...
import { fetchActiveComp_team, fetchSubmitComp_team} from '../../../api/activeBro/fetchHome'
...
const InCompetition_team = () => {
    ...
    useEffect(()=>{
      const getData = async () => {
        const response = await fetchActiveComp_team(compUuid)
    ...
    const getFontSize = (name) => {
      ...
      const handleSumbitClicked = async () => {
        if (isValidScore(teamScore)){
          const response = await fetchSubmitComp_team(compUuid, teamScore)
          if (response.ok){
            location.reload()
          }
        }
